...
{!! Form::open(['url' => action([\App\Http\Controllers\ProductController::class, 'update'] , [$product->id] ), 'method' => 'PUT', 'id' => 'product_add_form',
    ...
    @component('components.widget', ['class' => 'box-primary'])
        <div class="row">
            ...
            <div class="clearfix"></div>
              ...
              <div class="col-sm-4 @if(empty($common_settings['enable_product_defult_discount'])) hide @endif">
                <div class="form-group">
                    {!! Form::label('discount', __('lang_v1.discount') . ':*') !!}
                    {!! Form::select('discount_id', $discounts, !empty($product->discount_id) ? $product->discount_id : '', ['class' => 'form-control select2']); !!}
                </div>
              </div>
            ...
            @if(!empty($common_settings['enable_product_multi_unit']))
              <div class="multi_unit_box @if($product->enable_serial_number == 1) hide @endif">
                <div class="col-sm-4">
                    <div class="form-group">
                        <br>
                        <label>
                            {!! Form::checkbox('use_multi_unit', 1, $product->use_multi_unit, ['class' => 'input-icheck', 'id' => 'use_multi_unit']); !!} <strong>@lang('product.use_multi_unit')</strong>
                        </label>
                ...
                <div class="clearfix"></div>
                <div class="col-md-6 multi_unit @if(empty($product->first_conversion_unit_id)) hide @endif">
                    <div class="row">
                        <div class="col-sm-6">
                          <div class="form-group">
                            ...
                            <div class="input-group">
                                @php
                                  if (!empty($product->first_conversion_unit_id)) {
                                    $first_conversion_unit = App\Unit::where('id', $product->first_conversion_unit_id)->pluck('actual_name', 'id');
                                  }else{
                                    $first_conversion_unit = [];
                                  }
                                @endphp
                                {!! Form::select('first_conversion_unit_id', $first_conversion_unit, !empty($product->first_conversion_unit_id) ? $product->first_conversion_unit_id : null, ['placeholder' => __('messages.please_select'),'class' => 'form-control select2']); !!}
                                <span class="input-group-btn">
                                    <button type="button" @if(!auth()->user()->can('unit.create')) disabled @endif class="btn btn-default bg-white btn-flat btn-modal" data-href="{{action([\App\Http\Controllers\UnitController::class, 'create'], ['quick_add' => true])}}" title="@lang('unit.add_unit')" data-container=".view_modal">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                {!! Form::text('first_conversion_unit_rate', !empty($product->first_conversion_unit_id)?$product->first_conversion_unit_rate:null, ['class' => 'form-control', 'id'=>'first_conversion_unit_rate', 'placeholder' => __('Conversion rate')]); !!}
                            </div>
                        </div>
                    ...
                    <div class="row">
                            ...
                            <div class="input-group">
                                @php
                                  if (!empty($product->second_conversion_unit_id)) {
                                    $second_conversion_unit = App\Unit::where('id', $product->second_conversion_unit_id)->pluck('actual_name', 'id');
                                  }else{
                                    $second_conversion_unit = [];
                                  }
                                @endphp
                                {!! Form::select('second_conversion_unit_id', $second_conversion_unit, !empty($product->second_conversion_unit_id) ? $product->second_conversion_unit_id : null, ['placeholder' => __('messages.please_select'), 'class' => 'form-control select2']); !!}
                                <span class="input-group-btn">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                {!! Form::text('second_conversion_unit_rate', !empty($product->second_conversion_unit_id)?$product->second_conversion_unit_rate:null, ['class' => 'form-control', 'id'=>'second_conversion_unit_rate', 'placeholder' => __('Conversion rate')]); !!}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-sm-4">
              <div class="form-group">
                  <br>
                  <label>
                      {!! Form::checkbox('enable_serial_number', 1, !(empty($product->enable_serial_number)) ? $product->enable_serial_number : false, ['class' => 'input-icheck','id'=>'enable_serial_number']); !!} <strong>@lang('lang_v1.enable_serial_number')</strong>
            ...
            <div class="col-sm-4 @if(empty($common_settings['enable_product_premade'])) hide  @endif">
              <div class="form-group">
                  <br>
                  <label>
                      {!! Form::checkbox('pre_made', 1, $product->pre_made, ['class' => 'input-icheck', 'id' => 'pre_made']); !!} 
    ...
    @if($product->pre_made == 1) @component('components.widget', ['class' => 'box-primary', 'id' => 'pre_made_div']) @else @component('components.widget', ['class' => 'box-primary hide', 'id' => 'pre_made_div']) @endif
      <div class="row">
          <div class="col-md-12">
                  ...
                  <div class="col-sm-8">
                      <div class="form-group">
                          <div class="input-group">
                              ...
                              @php
                                  $count = count($premade_details);
                              @endphp

                              <input type="hidden" id="product_row_count" value="{{$count}}">
                      ...
                      <table class="table table-condensed table-bordered table-th-green text-center table-striped" id="premade_entry_table">
                          ...
                          <thead>
                              <tr>
                                  ...
                                  <th>@lang( 'product.default_purchase_price')({{ __('product.exc_of_tax') }})</th>
                                  {{-- <th>@lang('product.default_purchase_price')({{ __('product.exc_of_tax') }})</th> --}}

                                  <th>@lang( 'lang_v1.profit_margin' )</th>

                                  <th>@lang( 'product.default_selling_price')<small>(@lang('product.inc_of_tax'))</small></th>
                                  <th>@lang( 'product.total_amount')<small>(@lang('product.inc_of_tax'))</small></th>
                          ...
                          <tbody>
                              @php
                                $total_quantity = 0;
                                $total_subtotal = 0
                                // total_subtotal_input
                              @endphp
                              @foreach ($premade_details as $premade_line)

                              @php
                                $total_quantity += $premade_line->quantity_ordered;
                                $total_subtotal += ($premade_line->sell_price_inc_tax*$premade_line->quantity_ordered);
                              @endphp
                                @include('product.partials.edit_premade_row', 
                                  [
                                    'product' => $premade_line, 
                                    'row_count' => $loop->index, 
                                    'sub_units' => !empty($premade_line->unit_details) ? $premade_line->unit_details : [],
                                    'action' => 'edit'
                                  ]
                                )
                              @endforeach
                  ...
                  <div class="pull-right col-md-5">
                      <table class="pull-right col-md-12">
                          ...
                          <tr>
                              ...
                              <td class="col-md-5 text-left">
                                  <span id="total_subtotal" class="display_currency">{{ $total_subtotal }}</span>
                                  <!-- This is total before purchase tax-->
                                  <input type="hidden" id="total_subtotal_input" value="{{ $total_subtotal }}"  name="total_before_tax">
                              </td>
                          </tr>
                      </table>
                  </div>
